...
const landingPage = () => {
    const featureList = features
        .map((feature, index) => <Feature key={index} {...feature} reverse={index % 2 !== 0} />);

    return (
        <>
            <section className="jumbotron">
                <div>
                    <h1 className="title">Покори немецкий с deutschnerd!</h1>
                    <p className="description">Deutschnerd — это эффективный сервис для нескучного
                        изучения немецкого языка. Более 20 000 000
                        человек во всем мире выбрали Deutschnerd.
                        Присоединяйся!
                    </p>
                    <Link className="start-learning-button" to="/main">Начать учиться!</Link>
                </div>
            ...
            <section className="statistics">Statistics</section>
        </>
    )
};
